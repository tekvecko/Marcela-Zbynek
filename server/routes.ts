...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Photo upload endpoint - temporarily disabled
  app.post("/api/photos/upload", uploadRateLimit, optionalAuth, async (req: AuthRequest, res) => {
    // Photo upload functionality is currently disabled
    return res.status(503).json({ 
      message: "Nahrávání fotek je momentálně dočasně vypnuto pro údržbu." 
    });
  });
